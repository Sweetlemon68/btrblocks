...
template <>
btrblocks::Vector<std::string_view> generateData(size_t size,
                                            size_t unique,
                                            size_t runlength,
                                            int seed) {
  static std::vector<std::string> data(size);
  std::mt19937 gen(seed);
  for (auto i = 0u; i < size; ++i) {
    auto number = std::to_string(gen() % unique);
    for (auto j = 0u; i < size && j != runlength; ++j, ++i) {
      data[i] = number;
    }
  }
  std::vector<std::string_view> data_views;
  std::transform(data.begin(), data.end(), std::back_inserter(data_views),
                 [](const std::string& s) { return std::string_view(s); });
  return btrblocks::Vector<std::string_view>(data_views);
}
// ------------------------------------------------------------------------------
template <typename T>
...
bool validateString(size_t size, unsigned col, const btrblocks::Chunk& input, const btrblocks::Chunk& output) {
  for (auto i = 0u; i != size; i++) {
    const auto input_view = input(col, i);
    const auto output_view = output(col, i);
    if (input_view.size() != output_view.size()) {
      std::cout << "length @" << i << " does not match; in " << input_view.size() << " vs out "
                << output_view.size() << std::endl;
      return false;
    }
    if (input_view != output_view) {
      std::cout << "value @" << i << " does not match;" <<
        input_view << " vs " << output_view << std::endl;
      return false;
    }
  }
  return true;
...
int main(int argc, char* argv[]) {
  ...
  size_t size = 640000;
  size_t runlength = getenv("runlength") ? atoi(getenv("runlength")) : 20;
  size_t unique = getenv("unique") ? atoi(getenv("unique")) : ((1 << 20) - 1);
  ...
  to_compress.addColumn({"strs", generateData<std::string_view>(size, unique, runlength, 39)});
  ...
  for (auto col = 0u; col != to_compress.columns.size(); ++col) {
    ...
    switch (to_compress.columns[col].type) {
      case ColumnType::INTEGER:
        check = check && validateData(size, reinterpret_cast<int32_t*>(orig.get()),
                             reinterpret_cast<int32_t*>(decomp.get()));
        break;
      case ColumnType::DOUBLE:
        check = check && validateData(size, reinterpret_cast<double*>(orig.get()),
                             reinterpret_cast<double*>(decomp.get()));
        break;
      case ColumnType::STRING:
        check = check && validateString(size, col, input, decompressed);
        break;
      default:
        UNREACHABLE();
    }
